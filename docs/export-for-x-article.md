...
Living guide for putting a site post into an **X Article**. **Current path: rich paste** — same as Substack. The Articles API path is **parked**.

| Related | Path |
|---------|------|
| **Active workflow** | [`docs/export-for-substack.md`](export-for-substack.md) — `export-absolute-md.mjs --rich` |
...
X Article body accepts formatted paste the same way Substack does. Plain markdown does not auto-render.
...
1. Set **title**.
2. **Paste** body (⌘V) — not “Match Style.”
3. Upload cover from `assets/covers/<slug>.jpg` (20:9).
4. Publish when ready.

Full detail, flags, and failure modes: [`export-for-substack.md`](export-for-substack.md).
...
**One source.** `content/posts/<slug>.md` stays relative. External surfaces get a disposable projection (rich clipboard now; API payload only if unparked). Never edit source for the editor; never treat `export/` as canon.
