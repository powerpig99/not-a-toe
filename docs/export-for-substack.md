...
3. **Substack editor**
   ...
   - **Cover / social image:** upload `assets/covers/<slug>.jpg` locally, or use the cover URL the script printed if Substack accepts remote images. Prefer re-upload so Substack holds its own copy (X/GitHub hotlink quirks).
...
2. [ ] Cover at `assets/covers/<slug>.jpg` if used; site pushed so cover URL resolves.
