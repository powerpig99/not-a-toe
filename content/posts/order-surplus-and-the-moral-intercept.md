...
[Symptom and cause in the narratives of progress](../symptom-and-cause-in-the-narratives-of-progress/) is that delay treated as engine: the loud register arrives late and is installed as the lever. Hope arriving is that register. The generating sequence stayed earlier.

## The instrument arrives with those already moving

The kitchen comment assumes that willingness and insight are already widely distributed, and that only the instrument is missing. The instrument arrives first with those who were already trying to move the problem whether a finished tool existed or not. Those who wait for the tool receive it as a finished object, use it more or less as shipped, and treat that use as authorship. Matching the next instruction to a need they were already distinguishing is the continuation that writes rather than waits.

That split is what a general technique usually produces. Most people take the new method as a black box — a treatment to accept or refuse, a hope to post. A few treat it as an interface they can extend: the next antigen set, the next indication, the next way of writing the instruction. The two uses sit in the same residue, continued at different densities. They are not a plan.
