...
graph LR
    subgraph OriginalState["【原初认知向量：高维探索】"]
        ...
        Space["【正交开放空间】<br>无限探索维度<br>零竞争，无挤压"]
        Reality -.-> Space
    end

    subgraph InvertedState["【制度化倒置：低维竞技场】"]
        Student["受规训的参赛者"]
        Register["低维评价寄存器<br>(分数、位次、课题经费、影响因子)"]
        OtherPeers["狭窄跑道上的同侪竞争者"]
        Student ==>|"争夺稀缺指标与排位<br>零和淘汰赛"| Register
        OtherPeers ==>|"相互踩踏与挤压"| Register
        WarZone["【拥挤的角斗场】<br>指标最大化，熄灭内在好奇"]
        Register -.-> WarZone
    end
...
graph LR
    subgraph OriginalState["[Primal Cognitive Vector: High-Dimensional Inquiry]"]
        ...
        Space["[Orthogonal Open Space]<br>Infinite degrees of freedom<br>Zero competition, endless frontier"]
        Reality -.-> Space
    end

    subgraph InvertedState["[Institutional Inversion: Low-Dimensional Arena]"]
        Student["Conditioned Competitor"]
        Register["Low-D Scalar Register<br>(Grades, percentiles, grants, impact factors)"]
        OtherPeers["Rival Silhouettes on Narrow Track"]
        Student ==>|"Scramble for scarce metrics<br>Gladiatorial zero-sum contest"| Register
        OtherPeers ==>|"Mutual crowding and attrition"| Register
        WarZone["[Crowded Gladiator Arena]<br>Metrics maximized, wonder extinguished"]
        Register -.-> WarZone
    end
...
```mermaid
graph TD
    HighDReality["【高维实在与真实心智】<br>连续的好奇、隐性直觉与独特突破"]
    
    HighDReality ==>|"官僚管理的大规模压缩需求"| LowDRegister
    
    subgraph LowDRegister["【低维寄存器替代】"]
        Metric1["单一数值：考试分数与位次百分比"]
        Metric2["排他排位：院校等级与阶层通行证"]
        Metric3["学术计数器：论文影响因子与引用指数"]
        Metric4["资金筹码：项目经费与行政头衔"]
    end
    
    LowDRegister ==>|"【因果倒置：古德哈特定律】<br>指标异化为唯一目标"| BehaviorDrift
    
    subgraph BehaviorDrift["【行为模式的系统性扭曲】"]
        DriftA["应试策略取代对知识的敬畏：只背考点"]
        DriftB["学术寻租取代对真理的求索：抱团互引"]
        DriftC["零和厮杀：将同侪视为挤占指标的对手"]
    end
...
```mermaid
graph TD
    HighDReality["[High-D Reality & Authentic Mind]<br>Analog curiosity, tacit intuition, unique breakthroughs"]
    
    HighDReality ==>|"Administrative Need for Standardized Control"| LowDRegister
    
    subgraph LowDRegister["[Register Substitution]"]
        Metric1["Scalar Proxy: Exam scores & GPA percentiles"]
        Metric2["Positional Status: Institutional pedigree & credentials"]
        Metric3["Academic Tally: Paper counts, impact factors, h-index"]
        Metric4["Power Counters: Grants captured & administrative chairs"]
    end
    
    LowDRegister ==>|"[Causal Inversion: Goodhart's Law]<br>Metric Becomes the Sole Objective"| BehaviorDrift
    
    subgraph BehaviorDrift["[Systemic Behavioral Drift]"]
        DriftA["Syllabus gaming replaces genuine understanding"]
        DriftB["Citation cartels replace empirical breakthroughs"]
        DriftC["Zero-sum hostility: Peers treated as adversaries"]
    end
...
graph LR
    ...
    subgraph AcademicCartel["【科学建制：权势分赃】"]
        Scholar["职场化学者"]
        Committee["基金评审与期刊学阀"]
        GrantPower["稀缺资源寄存器<br>(顶级期刊版面、重点项目经费)"]
        Scholar ==>|"顺从主流范式、互引站队<br>平庸增量，发表至上"| Committee
        Committee ==>|"分配经费与头衔<br>终身教职、学会帽子"| GrantPower
        Stagnation["【前沿停滞与范式锁死】<br>可复现性危机<br>垃圾文献堆积"]
...
graph LR
    ...
    subgraph AcademicCartel["[The Academic Cartel: Spoils System]"]
        Scholar["Careerist Academic"]
        Committee["Review Committees & Journal Gatekeepers"]
        GrantPower["Scarce Resource Registers<br>(Nature/Science slots, major grants)"]
        Scholar ==>|"Conformity, Salami Slicing, Politics"| Committee
        Committee ==>|"Allocates tenure, chairs, funding"| GrantPower
        Stagnation["[Frontier Stagnation & Paradigm Lock]<br>Crisis of reproducibility<br>Incremental noise maxed out"]
        GrantPower -.-> Stagnation
    end
...
```mermaid
graph TD
    subgraph Trap["[Trapped State: Lost in the Low-D Arena]"]
        TrapMind["Identified with external ranks & credentials"]
