...
You can benchmark crystallized capability. You can't benchmark the fluid. The attempt to do so is a contradiction—not a hard problem awaiting a clever solution, but a category error built into the project itself. [Escaping the sandbox stays inside the hold](../escaping-the-sandbox-stays-inside-the-hold/) is the sealed-test face when the densified field is already broader than the evaluation: what looks like breakout is a model that was never coextensive with the test. [Mistaking the expression for the intelligence](../mistaking-the-expression-for-the-intelligence/) is the same freeze when crystallized scores—human or machine—are equated with the open activity that draws and revises the hold.
